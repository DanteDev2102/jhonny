...
let intro = null;
let light = null;
let explosionObjectArray = [];
let explosionObjectArrayCounter = 0;
...
class App extends Application {
	...
	async load() {
		...
		for (i in this.scene.nodes) {
			let node = this.scene.nodes[i];
			if (node instanceof Planet) {
				let stopinje = random(-180, 180);
				if (i == 0 || i == 1 || i == 2 || i == 3)
					stopinje = random(-38, 100);
				const kot = stopinje * (Math.PI / 180);
				const rotatedX =
					Math.cos(kot) * node.translation[0] -
					Math.sin(kot) * node.translation[2];
				const rotatedZ =
					Math.sin(kot) * node.translation[0] +
					Math.cos(kot) * node.translation[2];
	...
	update() {
		if (start) {
			...
			if (explosionObjectArray.length) {
				for (let i in explosionObjectArray) {
...
document.body.onkeyup = function (e) {
	if (e.keyCode == 32) {
		if (canShoot) {
			...
			metek.shoot();
			let cooldownHUD = document.querySelector('#cooldown');
			cooldownHUD.innerHTML = 'Charging...';

			let check = setInterval(function () {
				poslusanje();
			}, 17);
			let poslusanje = function () {
				if (metek.zadel) {
	...
	if (e.keyCode == 80) {
		...
		let time = 60;
		let scoreHUD = document.querySelector('#score');
		let timeHUD = document.querySelector('#time');
...
let animirajEksplozijo = function (unicenPlanet) {
	let eksplozija = null;
	let scalePlaneta = unicenPlanet.scale[0];
	let id = metek.zadetek.id;
	--id;
	setTimeout(function () {
...
let eksplodiraj = function (unicenPlanet) {
	let explozija = new Explosion(builder.spec);
...
let povecujEksplozijo = function (explozija, scalePlaneta) {
	let a = 1.5;
	let boom = function () {
		//console.log(explozija.scale[0]);
		vec3.scale(explozija.scale, explozija.scale, a);
		a = Math.pow(a, 0.891);
		if (explozija.scale[0] > scalePlaneta * 1.2) {
			clearInterval(povecajKroglo);
			console.log('good');
		}
	};
	let povecajKroglo = setInterval(function () {
		boom();
	}, 16);
};
let zmanjsujEksplozijo = function (explozija) {
	let a = 0.9999;
	let antiBoom = function () {
		vec3.scale(explozija.scale, explozija.scale, a);
		a = Math.pow(a, 1.01);
		if (explozija.scale[0] < 0.01) {
			clearInterval(zmanjsajKroglo);
			scene.deleteNode(explozija);
			explosionObjectArray[explozija.id] = null;
		}
	};
	let zmanjsajKroglo = setInterval(function () {
		antiBoom();
	}, 16);
};

let uniciPlanet = function (unicenPlanet) {
	let skrciPlanet = setInterval(function () {
		kolaps();
	}, 16);
	let a = 0.999;
	let kolaps = function () {
		...
		a = Math.pow(a, 1.1);
		if (unicenPlanet.scale[0] < 0.01) {
			scene.deleteNode(unicenPlanet);
			clearInterval(skrciPlanet);
		}
	};
};
